...
export default class CourseController extends ApiControllerAbstract implements DataManagementService<Course> {
  ...
  public get(id: string): Promise<Course> {
    return this.api.getCourse({courseId: id} as GetCourseRequest)
      .then((course: Course) => {
        return this.api.getAlias({uuid: course.id} as GetAliasRequest)
  ...
  public getWithAlias(alias: string): Promise<Course> {
    return this.api.getUUID({alias})
      .then((response: AliasResponse) => {
        if (response.objectType === ObjectType.COURSE) {
          return this.api.getCourse({courseId: response.uuid} as GetCourseRequest)
            .then((course: Course) => {
              course.alias = response.alias;
              return course;
            })
  ...
  public create(course: Course): Promise<string> {
    return this.api.createCourse({course} as CreateCourseRequest)
      .then((response: string) => {
        course.id = response;
        return this.api.createAlias({objectId: course.id, objectType: ObjectType.COURSE})
